...
/*   Updated: 2014/09/27 16:27:35 by takiapo          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void				*find_place(t_block *list, int size)
{
	...
	while (temp)
	{
		if (temp->freed  && temp->size - ABS(size) - g_size >= 0)
			return (downsize(temp, size, temp->size - ABS(size) - g_size));
		j += (temp->size + g_size);
		if (temp->next == NULL)
			list = temp;
		temp = temp->next;
...
void					*malloc(size_t size)
{
	...
	if (size <= 48)
		...
		temp = large_malloc(&g_large_malloc, size = -size, temp);
